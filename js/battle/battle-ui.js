...
function renderResource(s){
  const view=formalResourceView(s),bar=overlay.querySelector('#bHeroEnergy'),text=overlay.querySelector('#bHeroEnergyText');
  bar.style.width=pct(view.value,view.max)+'%';
  text.textContent=`${view.label} ${Math.round(view.value)} / ${Math.round(view.max)}${view.secondary?`　${view.secondary.label} ${Math.round(view.secondary.value)} / ${Math.round(view.secondary.max)}`:''}${s.formal?.combo?`　🔴 ${s.formal.combo}/3`:''}`;
}
function renderState(s){
  if(!overlay||!s)return;
  const target=s.enemies.find(x=>x.uid===s.selectedEnemyUid)||s.enemies.find(x=>x.hp>0),hpH=pct(s.heroHp,s.hero.maxHp),hpE=pct(target?.hp,target?.maxHp);
  overlay.querySelector('#bHeroHp').style.width=hpH+'%';overlay.querySelector('#bHeroHpText').textContent=`❤️ ${Math.round(s.heroHp)} / ${s.hero.maxHp}`;
  renderResource(s);
  ...
  overlay.querySelector('#bEnemyName').textContent=target?.name||'敵人';overlay.querySelector('#bHeroStatus').innerHTML=statusHTML(s.hero);overlay.querySelector('#bEnemyStatus').innerHTML=statusHTML(target);overlay.querySelector('#bRound').textContent=`ROUND ${s.round}`;
  overlay.querySelector('#bLog').innerHTML=s.logs.slice(-9).map(x=>`<div>${x}</div>`).join('');
...
async function presenter(event,s){
  ...
  const actor=event.side==='hero'?overlay.querySelector('[data-battle-actor="hero"]'):overlay.querySelector(`[data-enemy-uid="${event.attackerUid}"]`),target=event.side==='hero'?overlay.querySelector(`[data-enemy-uid="${event.targetUid}"]`):overlay.querySelector('[data-battle-actor="hero"]');
  if(!actor||!target)return;
  if(event.side==='hero'){const img=actor.querySelector('img');img.src=assetUrl(heroSpritePath('attack'));await actor.animate([{transform:'translate(-50%,-100%)'},{transform:'translate(calc(-50% + 70px),-100%)',offset:.48},{transform:'translate(-50%,-100%)'}],{duration:420,easing:'ease-out'}).finished.catch(()=>{});img.src=assetUrl(heroSpritePath('idle'))}
...
export async function openBattle(battleEngine,closeCallback){
  ...
  engine.subscribe((state,event)=>{if(event.type==='item-used'&&event.stackKey)consumeBattleConsumable(event.stackKey);renderState(state);if(event.type==='end')settle(event.result)});
  renderState(s);requestAnimationFrame(()=>applySceneLayout(overlay));
  ...
  overlay.querySelector('#bSkills').onclick=()=>{
    const box=overlay.querySelector('#bSubmenu');
    box.innerHTML=skills.map((sk,i)=>`<button class="battle-choice-v2" data-skill-index="${i}"><strong>${sk.icon||'✨'} ${sk.name}　${sk.formalClass?formalCostIcon(engine.state,sk):'⚡'}${Number(sk.cost)||0}</strong><span>${sk.description||''}</span></button>`).join('');
    box.classList.toggle('show');
    box.querySelectorAll('[data-skill-index]').forEach(b=>b.onclick=async()=>{const skill=skills[Number(b.dataset.skillIndex)];if(!skill)return;box.classList.remove('show');const used=await engine.playerAction({...skill,kind:'skill'},presenter);if(used&&!skill.formalClass)recordBeginnerSkillUsage(skill.id)});
  };
  ...
  overlay.querySelector('#bCancel').onclick=closeBattle;
}
