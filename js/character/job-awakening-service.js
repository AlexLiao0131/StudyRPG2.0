...
import { JOB_RULES, JOB_AWAKEN_CONFIG, BEGINNER_SKILL_JOB_WEIGHTS, FORMAL_CLASS_SKILL_IDS } from './job-rules.js';

const result=(ok,message,data={})=>({ok,message,...data});
const STAT_KEYS=['str','agi','int','will','virtue'];
...
export function approvedAwakeningRecords(game=getGame()){
  return (game.taskRecords||[]).filter(r=>
    r?.status==='completed'&&r.approvalStatus==='approved'&&approvalRatio(r)>0&&r.voluntaryChallenge!==true
  );
}
...
function totalGrowth(game){return STAT_KEYS.reduce((n,k)=>n+jobGrowthValue(k,game),0)}
...
function dualQualified(rule,game){
  ...
  const[a,b]=rule.cores,sa=growthShare(a,game),sb=growthShare(b,game),va=jobGrowthValue(a,game),vb=jobGrowthValue(b,game),
    balance=Math.max(va,vb)>0?Math.min(va,vb)/Math.max(va,vb):0;
...
function coreShare(rule,game){
  return rule.kind==='dual'?Math.min(...rule.cores.map(k=>growthShare(k,game))):growthShare(rule.core,game);
}
function beginnerSkillAffinity(jobName,game){
  const usage=game.hero?.skillUsage||{},ids=Object.keys(BEGINNER_SKILL_JOB_WEIGHTS),total=ids.reduce((n,id)=>n+(Number(usage[id])||0),0);
  if(total<=0)return 0;
...
export function currentJobCandidate(game=getGame()){
  const dual=JOB_RULES.filter(j=>j.kind==='dual'&&dualQualified(j,game)).map(j=>({j,score:blendedScore(j,game)})).sort((a,b)=>b.score-a.score);
  if(dual.length&&(dual.length===1||dual[0].score>=dual[1].score*JOB_AWAKEN_CONFIG.leadRatio))return dual[0].j;
  const single=JOB_RULES.filter(j=>j.kind==='single'&&coreShare(j,game)>=JOB_AWAKEN_CONFIG.minCoreGrowthShare).map(j=>({j,score:blendedScore(j,game)})).sort((a,b)=>b.score-a.score);
...
function recordCandidateSnapshot(game,candidate){
  ...
  const week=semesterWeekIndex(localDateString()),history=game.hero.jobCandidateHistory||(game.hero.jobCandidateHistory=[]),found=history.find(x=>Number(x.week)===Number(week));
  if(found){found.job=candidate.name;found.date=localDateString()}else history.push({week,job:candidate.name,date:localDateString()});
  return true;
}
...
function strongestFallback(game){
  const ranked=JOB_RULES.map(j=>({j,score:blendedScore(j,game),qualified:j.kind==='dual'?dualQualified(j,game):coreShare(j,game)>=JOB_AWAKEN_CONFIG.minCoreGrowthShare})).sort((a,b)=>b.score-a.score);
  return(ranked.find(x=>x.qualified)||ranked[0]||{}).j||null;
}
function grantFormalClassSkills(hero,className){
  const ids=FORMAL_CLASS_SKILL_IDS[className]||[];
  if(!ids.length)return;
  hero.knownSkills=[...new Set([...(hero.knownSkills||[]),...ids])];
  hero.skills=[...new Set([...(hero.skills||[]),...ids])];
  const current=(hero.equippedSkills||[]).filter(id=>ids.includes(id));
  if(!current.length)hero.equippedSkills=ids.slice(0,4);
  else hero.equippedSkills=current.slice(0,4);
}

export function jobEvaluationReport(game=getGame()){
  const total=totalGrowth(game),candidate=currentJobCandidate(game);
  const growth=Object.fromEntries(STAT_KEYS.map(k=>[k,{value:jobGrowthValue(k,game),share:growthShare(k,game)}]));
  const rows=JOB_RULES.map(rule=>{
    const qualified=rule.kind==='dual'?dualQualified(rule,game):coreShare(rule,game)>=JOB_AWAKEN_CONFIG.minCoreGrowthShare;
    const cores=rule.kind==='dual'?rule.cores:[rule.core],values=cores.map(k=>jobGrowthValue(k,game)),shares=cores.map(k=>growthShare(k,game));
    const balance=rule.kind==='dual'&&Math.max(...values)>0?Math.min(...values)/Math.max(...values):1;
    return{
      name:rule.name,kind:rule.kind,cores:[...cores],secondary:rule.secondary,qualified,
      coreValues:values,coreShares:shares,balance,
      realityScore:Math.max(0,jobScore(rule,game)),
      skillAffinity:beginnerSkillAffinity(rule.name,game),
      blendedScore:Math.max(0,blendedScore(rule,game)),
      isCandidate:candidate?.name===rule.name
    };
  }).sort((a,b)=>b.blendedScore-a.blendedScore);
  return{
    totalGrowth:total,growth,rows,candidate,
    skillUsage:{...(game.hero?.skillUsage||{})},
    candidateStableWeeks:candidate?stableCandidateWeeks(candidate.name,game):0
  };
}
...
export function evaluateNaturalAwakening(){
  ...
  if(!chosen){
    if(changed)update(()=>{});
    return result(false,awakeningDiagnostic(game).reason,{awakened:false,diagnostic:awakeningDiagnostic(game)});
  }

  hero.heroClass=chosen.name;
  hero.jobAwakened=true;
  hero.jobPassive=chosen.passive;
  grantFormalClassSkills(hero,chosen.name);
  update(()=>{});
  return result(true,`✨ ${hero.name} 自然覺醒為【${chosen.name}】！已取得8個職業技能，可裝備其中4招。${safety?'（最晚覺醒保險判定）':''}`,{
    awakened:true,className:chosen.name,safety,diagnostic:awakeningDiagnostic(game)
  });
}
