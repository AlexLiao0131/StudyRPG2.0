...
const views={status:()=>renderStatus(root),tasks:()=>renderTasks(root,render),dungeon:()=>renderDungeon(root,render),shop:()=>renderShop(root),lottery:()=>renderLottery(root),bag:()=>renderBag(root),social:()=>renderSocial(root),gm:()=>renderGM(root,render)};
