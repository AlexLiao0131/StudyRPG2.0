...
function normalizeTaskInput(input={},existing=null){
  ...
  return {
    ...
    countForBossOnNonSchoolDay:!!input.countForBossOnNonSchoolDay,
    ...
    scoreMax:Math.max(1,Number(input.scoreMax)||100),allowCorrection:input.allowCorrection!==false,correctionExp:Math.max(0,Number(input.correctionExp)||0)
  };
}
...
export function saveTaskDefinition(input,{editingId='',assignAll=false}={}){
  ...
  try{
    update(()=>{
      ...
      if(!existing&&assignAll){for(const p of family.profiles){if(p.id===family.activeProfileId)continue;p.data.tasks=Array.isArray(p.data.tasks)?p.data.tasks:[];p.data.tasks.push({...clone(next),id:uid()});p.data.examCompletionTargets={};}}
    });
...
export function removeTaskDefinition(id){let mode='';update(()=>{const g=getGame(),t=(g.tasks||[]).find(x=>String(x.id)===String(id));if(!t)return;const hasHistory=(g.taskRecords||[]).some(r=>String(r.taskId)===String(id));if(hasHistory){t.archived=true;t.active=false;t.archivedAt=new Date().toISOString();t.activeUntil=t.activeUntil||localDateString();mode='archived'}else{g.tasks=g.tasks.filter(x=>String(x.id)!==String(id));mode='deleted'}invalidateExamTargets(g)});return mode?result(true,mode==='archived'?'任務已有歷史紀錄，已改為封存。':'任務已刪除。',{mode}):result(false,'找不到任務。')}
export function toggleTaskDateCancellation(id,dateStr=localDateString()){if(!validDate(dateStr))return result(false,'日期格式不正確。');const g=getGame(),t=(g.tasks||[]).find(x=>String(x.id)===String(id));if(!t)return result(false,'找不到任務。');const cancelled=Array.isArray(t.cancelledDates)&&t.cancelledDates.includes(dateStr);if(!cancelled){if((g.taskRecords||[]).some(r=>String(r.taskId)===String(id)&&r.date===dateStr&&r.status==='completed'))return result(false,'這天已有完成紀錄，不能再用取消排程移除；請用家長核定處理成果。');if(!isTaskScheduled(t,dateStr))return result(false,'這個任務在指定日期原本就沒有排程。')}update(()=>{const live=(getGame().tasks||[]).find(x=>String(x.id)===String(id));live.cancelledDates=Array.isArray(live.cancelledDates)?live.cancelledDates:[];live.cancelledDates=cancelled?live.cancelledDates.filter(d=>d!==dateStr):[...new Set([...live.cancelledDates,dateStr])].sort();invalidateExamTargets(getGame())});if(dateStr===localDateString())reconcileDailyBalanceSnapshot(dateStr,{allowDecrease:true,reason:cancelled?'task-date-restored':'task-date-cancelled'});return result(true,cancelled?`已恢復 ${dateStr} 的任務。`:`已取消 ${dateStr} 的任務。`,{cancelled:!cancelled})}
