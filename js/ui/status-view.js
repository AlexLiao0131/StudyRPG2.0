...
function timelineHTML(){
  ...
  const node=e=>{
    const st=calendarStatus(e),mark=st==='done'?'✓ ':st==='overdue'?'❗ ':'',current=e.date===today?' current':'';
    const stateClass=st==='overdue'?'current':st==='done'?'done':'';
    return `<div class="timeline-node ${stateClass}${current}" data-calendar-id="${esc(e.id)}" role="button" tabindex="0" aria-label="${esc(e.name)}">
      <div class="icon">${calendarIcon(e.type)}</div><div>${mark}${esc(e.name)}</div>
      <div class="date">${esc(String(e.date||'').slice(5).replace('-','/'))} ${esc(calendarTimeText(e))}</div>
    </div>`;
  };
  const snake=rows.map((row,ri)=>{
    const content=row.map((e,i)=>`${i?'<div class="timeline-line"></div>':''}${node(e)}`).join(''),turn=ri<rows.length-1?`<div class="timeline-turn ${ri%2?'left':''}"></div>`:'';
    return`<div class="timeline-snake-row ${ri%2?'reverse':''}">${content}</div>${turn}`;
  }).join('');
  return `<div class="card timeline-card">
    <h3>🗺️ 學期冒險 ${overdue?`<span class="pill bad">❗${overdue} 件未完成</span>`:''}</h3>
    <div class="small timeline-help">完整顯示全部事件；路線會依畫面寬度自動折返。點事件可看細節、完成或修改自己的行程。</div>
    <div class="timeline-snake">${snake||'<div class="small">目前沒有行程。孩子也可以新增自己的行程。</div>'}</div>
    <button id="addMyCalendarEvent" class="action-button blue timeline-add">＋ 我的行程</button>
  </div>`;
}
...
export function renderStatus(root){
  ...
  statusClockTimer=setInterval(()=>{
    const el=root.querySelector('#statusTime');
    if(!el){clearInterval(statusClockTimer);statusClockTimer=null;return}
  ...
  root.querySelectorAll('[data-calendar-id]').forEach(node=>{
    const open=()=>showCalendarEvent(node.dataset.calendarId);
...
function showCalendarEvent(id){
  const e=calendarEventById(id);if(!e)return;
  ...
  m.id='calendarEventModal';m.className='modal-overlay';
  m.innerHTML=`<div class="modal-box calendar-event-modal">
    <h2>${calendarIcon(e.type)} ${esc(e.name)}</h2>
    <div class="grid2">
      <div><small>日期</small><b>${esc(e.date||'未設定')}</b></div>
      <div><small>時間</small><b>${esc(calendarTimeText(e)||'未設定')}</b></div>
      <div><small>對象</small><b>${esc(calendarTargetText(e))}</b></div>
      <div><small>狀態</small><b class="${st==='overdue'?'bad':st==='done'?'good':''}">${esc(calendarStatusText(e,profileId))}</b></div>
    </div>
    ${e.note?`<div class="card"><b>附註</b><div class="calendar-note">${esc(e.note)}</div></div>`:''}
    <div class="row">
      ${e.requireComplete===false?'':`<button id="calendarToggleDone" class="action-button ${st==='done'?'':'primary'}">${st==='done'?'↩️ 恢復未完成':'✅ 標記完成'}</button>`}
      ${mine?'<button id="calendarEditMine" class="action-button blue">✏️ 修改我的行程</button>':''}
      <button id="calendarClose" class="action-button">關閉</button>
    </div>
  </div>`;
  document.body.appendChild(m);
  const close=()=>m.remove();
  m.addEventListener('click',ev=>{if(ev.target===m)close()});
  m.querySelector('#calendarClose').onclick=close;
  m.querySelector('#calendarToggleDone')?.addEventListener('click',()=>{close();const r=toggleCalendarCompletion(id,profileId);if(!r.ok)alert(r.message)});
  m.querySelector('#calendarEditMine')?.addEventListener('click',()=>{close();kidEditCalendarEvent(id)});
}

function kidAddCalendarEvent(){
  const name=prompt('行程名稱');if(name===null||!name.trim())return;
  const date=prompt('日期 YYYY-MM-DD',localDateString());if(date===null)return;
  const startTime=prompt('開始時間 HH:MM（全天請留空）','');if(startTime===null)return;
  const endTime=startTime?(prompt('結束時間 HH:MM','')??''):'';
  const note=prompt('附註（可留空）','');if(note===null)return;
  const r=addChildCalendarEvent({name,date,startTime,endTime,note});
  if(!r.ok)alert(r.message);
}

function kidEditCalendarEvent(id){
  const e=calendarEventById(id);if(!e)return;
  const name=prompt('修改名稱',e.name||'');if(name===null)return;
  const date=prompt('修改日期 YYYY-MM-DD',e.date||localDateString());if(date===null)return;
  const startTime=prompt('開始時間 HH:MM（全天留空）',e.startTime||'');if(startTime===null)return;
  const endTime=startTime?(prompt('結束時間 HH:MM',e.endTime||'')??''):'';
  const note=prompt('附註',e.note||'');if(note===null)return;
  const r=editChildCalendarEvent(id,{name,date,startTime,endTime,note});
  if(!r.ok)alert(r.message);
}
